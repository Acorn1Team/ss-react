...
function ChatInput({ onSendMessage }) {
  ...
  const handleCategorySelect = (category) => {
    ...
    axios
      ...
      .then((res) => {
        if (res.data.result) {
          setIsModalOpen(false); // 카테고리 선택 후 모달 닫기
          console.log("Category selected and chat room created.");
        }
      })
      .catch((err) => {
        console.log(err);
    ...
    axios
      ...
      .then((res) => {
        if (res.data.chatNo) {
          ...
          setChatNo(res.data.chatNo.no);
          setChats(res.data.chats);
          setIsModalOpen(false); // 모달을 열지 않음
        } else if (res.data.create) {
          // 새로운 방이 필요할 경우
          setChatNo(res.data.create);
          setIsModalOpen(true); // 모달을 열어 카테고리 선택을 유도
        }
      })
      .catch((err) => {
        console.log(err);
